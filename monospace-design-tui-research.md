...
## 8. Workflow archetypes: task-flow patterns across TUI history

The previous seven vectors examine the spatial, chromatic, and interactive vocabulary of text-mode interfaces — what appears on a single screen at a single moment. But users do not operate on single screens. They move through sequences of screens to accomplish tasks: creating a resource, reviewing a queue, configuring a system, diagnosing an anomaly. The patterns governing these multi-screen sequences — workflow archetypes — have their own deep history, distinct from but complementary to the screen-level archetypes documented above. This section traces those patterns from CUA's formalized dialog sequences through modern TUI applications, identifying a taxonomy of eight recurring workflow archetypes with concrete historical precedent.

### Dialog sequences and task-flow models in CUA

CUA did not merely specify widget layout; it specified how panels relate to each other across time. The 1987 Entry Model defined the simplest task flow: full-screen panels linked by F7 (backward) and F8 (forward) in a strictly linear sequential chain. A user entering purchase orders in an AS/400 application would advance through header panel → line items panel → review panel → confirmation panel, with no lateral branching. This was the wizard primitive, predating Microsoft's formalization by nearly a decade.

CUA '89 (SC26-4581) introduced the primary/secondary/dialog window hierarchy, formalizing a hub-and-spoke navigation model. A primary window served as the persistent home base — the "hub" — from which the user launched secondary windows for specific subtasks and dialog windows for parameter entry. The primary window remained visible (though inactive) while secondary windows occupied the foreground. Returning to the primary window from any spoke required only F3 (Exit) or F12 (Cancel), collapsing the spoke. CUA used the term "dialog sequence" for a chain of panels within a single spoke and "panel flow" for the overall navigation graph.

The 1991 Workplace Model, documented across SC34-4289 (Object-Oriented Interface Design) and SC34-4290 (Workplace Environment), represented CUA's most ambitious task-flow contribution. It replaced the action-first model ("choose a verb, then select a noun") with an object-action paradigm ("select a noun, then choose a verb"). This inversion restructured navigation entirely. Users no longer launched applications; they opened objects. Each object type exposed its own set of applicable actions through context menus. Property notebooks — tabbed containers of settings pages — provided a standardized configuration workflow: open any object's properties, navigate tabs laterally, modify settings, apply or cancel. Container objects (folders, databases) supported drill-down navigation: open a container to see its contents, open a contained object to see its properties. The Workplace Model thus originated three distinct workflow archetypes — drill-down (container navigation), configuration (property notebooks), and CRUD (universal object operations) — within a single coherent framework.

### OS/2 Workplace Shell task flows

The OS/2 Workplace Shell (WPS), shipping with OS/2 2.0 in 1992, implemented the CUA Workplace Model as a fully realized desktop environment. Its task-flow innovations were structural rather than cosmetic. The WPS enforced a data-oriented paradigm: the fundamental unit of interaction was the document, not the application. Double-clicking a spreadsheet opened it directly; the application was an invisible intermediary. This eliminated an entire workflow step — the "launch application, then open file" sequence that dominated DOS and early Windows — and replaced it with a direct object-action model.

Every WPS object, regardless of type, supported a universal set of operations accessible through a consistent right-click context menu: Open, Copy, Move, Delete, Create Shadow (symlink equivalent), and Properties. This universality meant that users learned one workflow — select object, choose operation — and applied it everywhere. The CRUD archetype (create, read, update, delete) was not an abstraction; it was the literal interaction model for every object on the desktop.

Property notebooks in the WPS were tabbed settings dialogs accessible from any object's context menu. A printer object's notebook contained tabs for output, printer driver, job defaults, and access control. A folder's notebook controlled sort order, display format, background color, and window position. The configuration workflow was identical regardless of object type: right-click → Properties → navigate tabs → modify → close. This consistency meant that configuring a network adapter and configuring a desktop folder followed the same task-flow pattern, differing only in the specific settings exposed.

WPS navigation was spatially flat with drill-down depth. Folders opened as independent windows at user-chosen positions; there were no tree views or hierarchical sidebars. Users drilled into nested folders by opening successive windows. The absence of wizard-style flows was deliberate: IBM's design philosophy held that expert users should access any setting directly, not be forced through a prescribed sequence. Wizards appeared later in OS/2 Warp 4 (1996) as "assistants" — an acknowledgment that infrequent tasks benefited from guided flows even in an object-action environment.

### Turbo Vision application flow patterns

Borland's Turbo Vision (1990) provided the toolkit primitives from which developers assembled multi-screen task flows. The key mechanism was `TDialog` combined with `execView()`: calling `execView(dialog)` suspended the current event loop, displayed the dialog modally, collected user input, and returned a command code (typically `cmOK` or `cmCancel`) when the dialog closed. Chaining multiple `execView()` calls created a wizard-like sequential flow: display panel A → collect input → display panel B → collect input → execute action. Each `execView()` call was effectively a wizard step primitive, with the calling code serving as the flow controller.

The Turbo Vision application hierarchy — `TApplication` → `TDesktop` → `TWindow` → `TDialog` — mapped naturally to a drill-down workflow model. The desktop provided the overview (monitor-respond pattern), open windows provided document-level context (CRUD pattern), and dialogs provided action-level detail (wizard or configuration pattern). MDI (Multiple Document Interface) support, with Alt+number shortcuts to switch between open windows, enabled lateral navigation between parallel tasks — editing one file while referencing another, or comparing two datasets side by side.

The `TStatusLine` component changed its displayed key bindings based on which view held focus, establishing a context-switching pattern that anticipated lazygit's panel-dependent key model by three decades. When focus moved from the editor to a dialog, the status line updated to show dialog-relevant keys; when a menu opened, it showed menu-navigation keys. This dynamic status line served as both a workflow compass (indicating which phase of a task the user occupied) and a discoverability mechanism (showing available actions without requiring memorization).

### Norton Commander and the dual-pane workflow

Norton Commander's contribution to workflow design was the source-target paradigm. The dual-pane layout was not merely an aesthetic choice; it encoded a specific task-flow model. The active panel (highlighted border) designated the source — the set of objects being operated upon. The inactive panel designated the target — the destination for operations like copy (F5) and move (F6). Pressing Tab switched which panel served as source and which as target. This implicit source-target designation eliminated an entire dialog step: instead of "select files → press copy → navigate to destination → confirm," the workflow became "navigate target panel to destination → Tab to source panel → select files → F5." The destination was pre-designated by panel position.

The Orthodox File Manager (OFM) paradigm, formalized by Dr. Nikolai Bezroukov in his taxonomy of file managers, identified the defining characteristics of this workflow: dual symmetric panels, Tab-to-switch focus, F-key operations applied to the selection in the active panel with the inactive panel as implicit target, and a command line below both panels for direct shell access. The symmetry was essential — either panel could serve as source or target, and the panels were functionally identical, making the workflow bidirectional.

Norton Commander's quick-action workflow represented a second, complementary pattern: single-keystroke operations executed directly from the file list with no intermediate screens. F3 launched the built-in viewer on the highlighted file immediately. F4 launched the editor. F7 created a directory after a single-field dialog (directory name). F8 deleted after a yes/no confirmation. These operations followed a minimal-dialog model: zero or one intermediate screen between intent and execution. The quick-action pattern was the antithesis of the wizard — designed for frequent operations where the user possessed full domain knowledge and needed no guidance.

### Microsoft wizard paradigm origins

The wizard as a formal interaction pattern emerged from Microsoft's desktop publishing efforts. Microsoft Publisher (1991) included guided sequences for creating newsletters, flyers, and brochures — flows that walked users through template selection, content entry, and layout options in a prescribed order. These were not yet called "wizards," but they established the structural pattern: a sequence of panels with constrained forward/backward navigation and a final confirmation step.

Windows 95 (1995) adopted wizards systematically across the operating system. The Add Printer wizard, Internet Connection wizard, and hardware installation wizards all followed the same structure: a sequence of panels navigated by Next and Back buttons, with Cancel available at every step and Finish on the final panel. Kent Sullivan's CHI '96 paper, "The Windows 95 User Interface: A Case Study in Usability Engineering," documented the design rationale: wizards were intended specifically for tasks that users performed infrequently and where they lacked domain knowledge to make decisions without guidance.

Microsoft published formal wizard design guidelines in 1997, codifying the button pattern (Back / Next / Cancel on intermediate pages, Back / Finish / Cancel on the final page), the prohibition against branching logic visible to the user (branches should appear as a single linear sequence), and the requirement that Cancel undo all changes made during the wizard. The guidelines distinguished between "linear wizards" (strict sequential steps) and "branching wizards" (where answers on one page determine which page appears next, though the user always perceives a linear flow).

The Nielsen Norman Group validated the wizard pattern's effectiveness with a specific finding: "Wizards work well for processes that are performed only occasionally and where the user might not have all domain knowledge required." But NN/g also documented the wizard's limitation — expert users find wizards frustrating because they impose a fixed sequence on users who already know what they want. This tension between guided and flexible workflows became a recurring theme in HCI research, leading to recommendations for "flexible wizard" patterns that allow direct access to any step while maintaining the sequential default.

### Enterprise workflow patterns: SAP R/3

SAP R/3, deployed in thousands of enterprises from the mid-1990s, implemented the most complex task-flow architecture of any character-mode system. Its transaction-code model launched specific screen sequences (dynpro sequences, from "dynamisches Programm") that embodied distinct workflow archetypes. Transaction VA01 (Create Sales Order) was a wizard-like linear flow: header data → line items → scheduling → pricing → confirmation. Transaction VA03 (Display Sales Order) was a drill-down: enter order number → view header → drill into line items → drill into pricing detail. Transaction MM02 (Change Material Master) was a tabbed configuration workflow: select material → choose views (Basic Data, Purchasing, MRP, Accounting) → modify fields across tabs. Transaction SM21 (System Log) was a monitor-respond pattern: display continuously updating log entries, filter by time/severity/component, drill into individual entries for detail.

The SAP screen flow was governed by the PBO/PAI event cycle: Process Before Output (PBO) prepared the screen's data and field attributes before display, and Process After Input (PAI) processed user actions after they pressed Enter or a function key. This cycle — prepare, display, wait, process — was the fundamental rhythm of every SAP workflow, repeated at each step in a transaction's screen sequence. The PBO/PAI model enforced a strict request-response pattern: the server controlled which screen appeared next based on the user's input, making it impossible for users to skip steps or navigate out of sequence without explicit escape actions (F3 for back, /n for cancel).

SAP's Business Workflow engine (transaction SWDD) formalized workflow at the process level, defining typed steps: Activity (user performs a task), Send Mail (notification), Block (parallel execution), Decision (conditional branch), Loop, and Event Wait. These process-level steps mapped directly to UI-level workflow archetypes: an Activity step presented a transaction screen sequence (wizard or CRUD), a Decision step presented an approval form (review-approve), and Event Wait implemented monitor-respond at the process level. The Workflow engine, though operating above the UI layer, demonstrated that the same small set of workflow primitives recurred whether modeling business processes or user interface flows.

### Modern navigation pattern taxonomy

Contemporary design systems and HCI research have converged on a taxonomy of navigation models that encompasses and organizes the historical patterns documented above. Material Design identifies five navigation patterns: lateral (tabs, bottom navigation), forward (hierarchical drill-down), reverse (back navigation), and no-navigation (single-screen tasks). Apple's Human Interface Guidelines distinguish flat navigation (tab bars), hierarchical navigation (navigation controllers with push/pop), content-driven navigation (page controllers, games), and modal presentation (task-focused interruptions). These taxonomies describe the geometric shape of the navigation graph — the topology of how screens connect.

Academic HCI research provides additional structure. The Nielsen Norman Group's research on complex application design identifies three fundamental navigation structures: sequential (wizard), hierarchical (drill-down), and hub-and-spoke (dashboard with detail views). Their research on workflow design found that users strongly prefer flexible workflows that allow direct access to any step over strictly sequential ones, except when performing an unfamiliar task for the first time — confirming the original Microsoft wizard design rationale.

Van der Aalst's Workflow Patterns Initiative, a joint research program between Eindhoven University of Technology and Queensland University of Technology begun in 1999, catalogued over 40 workflow patterns at the process level. Several have direct UI manifestations: Sequence (wizard), Parallel Split (side-by-side panels), Exclusive Choice (conditional branching in a wizard), Deferred Choice (user-driven next-step selection in hub-and-spoke), and Milestone (progress indicators). The Workflow Patterns Initiative demonstrated that a small set of control-flow primitives — sequence, choice, parallelism, synchronization, iteration — generates the full space of workflow topologies, just as a small set of screen archetypes generates the full space of single-screen layouts.

Synthesizing these taxonomies against the historical record produces a mapping between navigation topology and workflow archetype:

- **Sequential/Linear topology → Wizard, Pipeline archetypes.** A chain of screens traversed in order. CUA '87 F7/F8 panel flow, Microsoft's Next/Back/Finish pattern, SAP's VA01 transaction.
- **Hub-and-Spoke topology → CRUD, Monitor-Respond archetypes.** A central screen from which the user launches into detail screens and returns. CUA '89 primary/secondary windows, SAP's SM21 log viewer.
- **Hierarchical/Tree topology → Drill-Down archetype.** Nested levels of increasing detail. CUA '91 container navigation, ranger's Miller columns, k9s resource navigation.
- **Flat/Lateral topology → Configuration archetype.** Peer screens accessible in any order. CUA '91 property notebook tabs, SAP MM02 material master views.
- **Funnel topology → Search-Act archetype.** Progressive filtering from broad to narrow. fzf's type-to-filter, any search-results-to-detail flow.
- **Queue topology → Review-Approve archetype.** Items presented in sequence for disposition. Email triage, pull request review, SAP Business Workflow Decision steps.

### Terminal workflow patterns in modern applications

Modern TUI applications, whether consciously or not, implement combinations of these workflow archetypes. Analyzing specific applications reveals how the historical patterns manifest in contemporary terminal software.

**lazygit** combines three workflow archetypes in a single interface. Its five context panels (Status, Files, Branches, Commits, Stash) form a hub — lateral navigation via bracket keys or number keys. Within each panel, selecting an item and pressing Enter drills into detail (drill-down). The Files panel supports a CRUD workflow: stage, unstage, discard, edit, commit. The Commits panel supports search-act: filter by author or message, then cherry-pick, revert, or rebase. The monitor-respond archetype appears in the Status panel, which reflects repository state and updates on filesystem changes. lazygit's context-panel model — where the status line and available keybindings change per focused panel — is the interaction mechanism that enables switching between these workflow archetypes without changing screens.

**k9s** implements a command-mode drill-down workflow. Typing `:pods`, `:deployments`, or `:services` navigates to a resource list (hub). Selecting a resource and pressing Enter, `d` (describe), or `l` (logs) drills into detail views. The Escape key pops back up one level. This is CUA '91 container navigation adapted for Kubernetes: clusters contain namespaces, namespaces contain resource types, resource types contain individual resources, resources have logs, events, and YAML definitions. The `:` command entry is a direct descendant of the CUA command line (`Command ===>` on the panel footer), filtered through Vim's ex-command model.

**htop and btop** are pure monitor-respond implementations. The screen displays a continuously refreshing system overview — CPU graphs, memory bars, process table — with no multi-screen navigation. The user monitors the display, identifies an anomaly (high CPU process, memory pressure), and responds with an in-place action: send signal (F9/k), renice, filter, sort. The entire workflow occurs on a single screen with modal overlays for action confirmation. btop extended this pattern with Braille-character graphs and configurable layout, but the workflow archetype remained unchanged from htop, and from the system-monitoring tools that preceded both.

**ranger** implements Miller columns drill-down, a three-column layout displaying parent directory, current directory, and file preview simultaneously. Navigation (arrow keys, hjkl) moves up and down within the current directory or drills into/out of subdirectories, with all three columns updating in concert. This simultaneous display of three hierarchical levels — context, focus, preview — collapses what would be a multi-screen drill-down workflow into a single-screen spatial representation. The workflow archetype is still drill-down, but the interaction cost is dramatically reduced because the user sees where they came from, where they are, and where they are going in a single glance.

**taskwarrior-tui** implements a review-approve workflow with queue semantics. Tasks are presented in a sorted list (the queue). The user reviews each task and takes a disposition action: mark complete, modify priority, defer, annotate, or delete. The queue model is evident in the interaction rhythm: examine item, decide, act, advance to next item. This is the same workflow pattern as email triage, pull request review, and SAP Business Workflow decision steps — a sequence of items requiring human judgment applied one at a time.
...
**Synthesis for Monospace TUI.** The named palettes in the Rendering Reference (§R3.4) — Monochrome, Commander, Amber Phosphor, Green Phosphor — are not inventions. They are direct encodings of these historical color systems into the standard's semantic role framework. The Monochrome palette implements Turbo Vision's `cpAppMonochrome` strategy. The Commander palette encodes the OS/2/Norton Commander blue-panel conventions. The phosphor palettes preserve the DEC terminal single-hue aesthetic. The Default Dark and Light themes apply M3's semantic-role architecture to the 256-color palette. All palettes enforce Apple HIG's color independence rule. The historical progression from 2-color to 16-color to 256-color to truecolor is not treated as obsolescence — it is the graceful degradation path (§5.6) that every compliant application must support.
...
### Workflow archetype taxonomy

Eight workflow archetypes emerge from the historical research across all eight vectors: **wizard** (linear sequential flow), **CRUD** (create-read-update-delete on objects), **monitor-respond** (observe continuous state, act on anomalies), **search-act** (progressive filtering to action), **drill-down** (hierarchical navigation into increasing detail), **pipeline** (staged data transformation), **review-approve** (queue-based disposition of items), and **configuration** (lateral navigation across peer settings). Each maps to a specific navigation topology — wizard and pipeline to sequential/linear, CRUD and monitor-respond to hub-and-spoke, drill-down to hierarchical/tree, configuration to flat/lateral, search-act to funnel, review-approve to queue — and each has deep historical precedent: CUA formalized dialog sequences, hub-and-spoke, and property notebooks; Norton Commander originated source-target CRUD and quick-action patterns; SAP R/3's transaction codes implemented all eight archetypes across its thousands of business processes.

The relationship between workflow archetypes and screen archetypes (the layout patterns documented in sections 1–7) is compositional: workflows compose screens. A wizard workflow assembles a sequence of form screens. A drill-down workflow chains list screens to detail screens. A monitor-respond workflow pairs a dashboard screen with modal action dialogs. A single workflow may traverse multiple screen archetypes — lazygit's commit workflow moves from a monitor screen (status panel) through a CRUD screen (file staging) to a form screen (commit message entry) and back. The screen archetypes define what appears at each moment; the workflow archetypes define how those moments connect across time. Designing a TUI application requires decisions at both levels: choosing which screen archetypes to implement and choosing which workflow archetypes govern the transitions between them.

### The Monospace TUI design token vocabulary

Drawing from all eight research vectors, the core Monospace TUI token set emerges:
...
This token vocabulary provides Monospace TUI with a principled foundation that honors the precision of CUA and Turbo Vision, adopts the systematic design thinking of Material Design 3 and Apple HIG, leverages the full capability of modern terminal emulators, and structures multi-screen task flows around historically validated workflow archetypes — while keeping keyboard efficiency and accessibility as non-negotiable architectural constraints.