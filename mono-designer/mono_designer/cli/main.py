...
def project(path: Path):
    ...
    if isinstance(artifact, ScreenSpec):
        ...
        typer.echo(
            f"Projection for {artifact.artifact_type} not yet implemented.", err=True
        )
...
def lint(
    path: Path = typer.Argument(
        ..., help="Path to the directory containing design artifacts"
    ),
    schema_path: Path = typer.Option(
        None, help="Path to the JSON schema file for level 1 validation"
    ),
):
    """
    ...
    if not schema_path.exists():
        typer.echo(
            f"Warning: JSON schema not found at {schema_path}. Level 1 validation will be skipped.",
            err=True,
        )
    ...
    for r in results:
        if r.level == "error":
            error_count += 1
            typer.secho(
                f"[ERROR] {r.code}: {r.message} ({r.file_path})", fg=typer.colors.RED
            )
        else:
            warning_count += 1
            typer.secho(
                f"[WARNING] {r.code}: {r.message} ({r.file_path})",
...
def revise(path: Path, patch_json: str):
    ...
    try:
        ...
        typer.echo(
            f"Successfully revised {artifact.artifact_type} '{artifact.id}' at {path}"
        )
