                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            ...
                            if content and isinstance(content, dict):
                                ...
                                kind = content.get("kind") or content.get(
                                    "artifact_type"
                                )
...
class Linter:
    ...
    def _is_suppressed(
        self, artifact: Dict[str, Any], code: str, context: WorkspaceContext = None
    ) -> bool:
    ...
    def lint_workspace(self, context: WorkspaceContext) -> List[LintResult]:
        ...
        for file_path, artifact in context.artifacts.items():
            ...
            if kind == "screen":
                ...
                focus_order = (
                    focus.get("focus_order", []) if isinstance(focus, dict) else []
                )
                ...
                for comp in components:
                    if isinstance(comp, dict) and comp.get("type") in interactive_types:
                        comp_id = comp.get("id")
                        if not focus_order and not self._is_suppressed(
                            artifact, "H002", context
                        ):
                            results.append(
                                LintResult(
                                    file_path=file_path,
                        ...
                        elif comp_id not in focus_order and not self._is_suppressed(
                            artifact, "H002", context
                        ):
                            results.append(
                                LintResult(
