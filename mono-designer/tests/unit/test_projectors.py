...
from mono_designer.models.workflow import WorkflowSpec, WorkflowStage, Checkpoint
...
def test_project_nav_ascii():
    spec = NavigationSpec(
        ...
        routes=[
            Route(
                id="r1",
                **{"from": "w1", "to": "w2", "type": "push", "trigger": "click_btn"},
            )
        ],
        rules=["Rule 1", "Rule 2"],
    )
...
def test_project_workflow_ascii():
    spec = WorkflowSpec(
        ...
        transitions=[],
        checkpoints=[Checkpoint(id="cp1", label="CP 1", stage_id="s1")],
